...
class Control_filter_Index_predictor():
    ...
    def __init__(self, MODEL_PATH, device, fs):
        ...
        # set the model
        model = Modified_ShufflenetV2(num_classes=15)
    ...
    def predic_ID(self, noise): # predict the noise index
        spectorgram = waveform_to_spectorgram(noise)
        spectorgram = spectorgram.to(self.device)
        spectorgram = spectorgram.unsqueeze(0)
        prediction = self.model(spectorgram)
...
def Control_filter_selection_2D(fs=16000, Primary_noise=None):
    # pretrained CNN model path
    MODEL_PATH = 'D:/Coding/Gavin/Selective_ANC_CNN/feedforwardnet_2D.pth'
